// 25組循跡自走車 穩定過彎版
// 修正：修正漏掉大括號、消除重複降速衝突、恢復合理的脫軌緩衝計時
...
bool ledState = LOW;                  
const int TRACK_SPEEDL = 100;     // 左直線基準速度
const int TRACK_SPEEDR = 100;     // 右直線基準速度
float Kp = 3.45;  // 比例常數 
float Kd = 2.76    ;  // 微分常數 
  ...
  switch (currentState) {
    ...
    case 3:
    {
      ...
      if (abs(error) >= 39) {
        ...
        // TT 馬達專用直角彎補償：強迫負責推進的外側輪用固定大扭力過彎，避免卡死
      ...
      if (isLost) {
        ...
        if (lostDuration > 3000) {
          leftSpd  = 0;    // 超過3秒：全停
          rightSpd = 0;
        } else if (lostDuration > 1000) { // 改為 1000ms 較符合實際賽道物理
          leftSpd  = -160; // 真正斷線 1 秒後才開始倒車救援
          rightSpd = -160;
        }
        // 0~1秒內：維持原轉向慣性（盲轉）去追黑線
      }
