/*
   ========================================================
    新增:防呆測試 循跡數值輸出(5路) + 小車動作監控 
    燈號腳位：紅 12, 綠 13, 藍 14
    馬達腳位：IN1~4 -> 16, 17, 18, 19
    循跡腳位：最左 34, 左 33, 中 32, 右 35, 最右 39
...
// 擴充為 5 路循跡
#define TRACK_LL_PIN 34   // 最左
#define TRACK_L_PIN  33   // 左
#define TRACK_M_PIN  32   // 中
#define TRACK_R_PIN  35   // 右
#define TRACK_RR_PIN 39   // 最右
...
void setup() {
  ...
  // 5 路感測器設定為輸入
  pinMode(TRACK_LL_PIN, INPUT);
  pinMode(TRACK_L_PIN, INPUT);
  pinMode(TRACK_M_PIN, INPUT);
  pinMode(TRACK_R_PIN, INPUT);
  pinMode(TRACK_RR_PIN, INPUT);
  ...
  switch (currentState) {
    ...
    case 2: //  就緒模式 + 循跡數值校正測試 
      ...
      if (currentMillis - lastTrackPrint > 300) {
        trackLL = digitalRead(TRACK_LL_PIN);
        trackL  = digitalRead(TRACK_L_PIN);
        trackM  = digitalRead(TRACK_M_PIN);
        trackR  = digitalRead(TRACK_R_PIN);
        trackRR = digitalRead(TRACK_RR_PIN);
        
        Serial.print("📡 [感測器數值] 最左:"); Serial.print(trackLL);
        Serial.print(" 左:"); Serial.print(trackL);
        Serial.print(" 中:"); Serial.print(trackM);
        Serial.print(" 右:"); Serial.print(trackR);
        Serial.print(" 最右:"); Serial.println(trackRR);
        lastTrackPrint = currentMillis;
      }
      break;

    case 3: // 自主導航模式 (紅燈 2Hz 閃爍 + 5路循跡自動駕駛)
      ...
      // 3. 決定馬達動作與記錄 (優先權：中間 > 內側 > 外側) 新想法
      // 依照循跡邏輯，優先處理「中間壓線」的狀況，確保小車盡可能保持在軌道中心。
      if (trackM == 1) {
        motorForward();   
        currentAction = 1; // 狀態 1：直走
      } 
      else if (trackL == 1) {
        motorLeft();       // 未來加 PWM 可改：左輪減速/右輪加速 (微左轉)
        currentAction = 2; // 狀態 2：微幅向左修正
      } 
      else if (trackR == 1) {
        motorRight();      // 未來加 PWM 可改：右輪減速/左輪加速 (微右轉)
        currentAction = 3; // 狀態 3：微幅向右修正
      } 
      else if (trackLL == 1) {
        motorLeft();       // 外側壓線，需大角度轉向 (急左轉)
        currentAction = 4; // 狀態 4：急向左修正
      }
      else if (trackRR == 1) {
        motorRight();      // 外側壓線，需大角度轉向 (急右轉)
        currentAction = 5; // 狀態 5：急向右修正
      }
      else {
        motorStop();      
        currentAction = 0; // 狀態 0：完全沒線 -> 煞車
      }

      // 4. 監控輸出：只有當「決定做的動作」跟「上一次」不一樣時，才印出文字
      if (currentAction != lastAction) {
        if (currentAction == 1)      { Serial.println(" 中間壓線 ➡️ 直走！"); }
        else if (currentAction == 2) { Serial.println("內左壓線 ➡️ 微幅向左修正！"); }
        else if (currentAction == 3) { Serial.println(" 內右壓線 ➡️ 微幅向右修正！"); }
        else if (currentAction == 4) { Serial.println(" 最左壓線 ➡️ 急左轉/原地甩尾！"); }
        else if (currentAction == 5) { Serial.println(" 最右壓線 ➡️ 急右轉/原地甩尾！"); }
        else if (currentAction == 0) { Serial.println(" 完全沒壓線 ➡️ 煞車尋軌..."); } 
        
        lastAction = currentAction; // 更新
      }
      break;
