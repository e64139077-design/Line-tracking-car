...
const int TRACK_SPEEDL = 75;     // 左直線基準速度 原130 
const int TRACK_SPEEDR = 75;     // 右直線基準速度 原118
  ...
  switch (currentState) {
    ...
    case 3: // 自主：開始循跡導航，用加權平均法 & PID控制
      { 
        ...
        if (s1 == 1) { sum += -40; active++; } 
        if (s2 == 1) { sum += -20; active++; }
        if (s3 == 1) { sum +=   0; active++; } 
        if (s4 == 1) { sum +=  20; active++; }
        if (s5 == 1) { sum +=  40; active++; }
...
void setMotorSpeed(int leftSpeed, int rightSpeed) {
  // 限制範圍
  leftSpeed = constrain(leftSpeed, -200, 250);
  rightSpeed = constrain(rightSpeed, -200, 250);
